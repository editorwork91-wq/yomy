...
async function sendTokenWithRetry(token:string,title:string,body:string,data:Record<string,string>,type:string,projectId:string,accessToken:string){
  const message = type === 'call'
    ? { token, data, android: { priority: 'HIGH', direct_boot_ok: true, ttl: '60s', collapse_key: `yomy-call-${data.call_id || 'incoming'}` } }
    : { token, notification: { title, body }, data, android: { priority: 'HIGH', notification: { channel_id: type === 'notification' ? 'yomy_default' : 'yomy_default', sound: 'default' }, ttl: '300s' } }

  let lastReason = 'FCM_UNKNOWN'
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (response.ok) return { sent: true, reason: '' }
      const errorBody = await response.json().catch(() => ({})) as Record<string, unknown>
      lastReason = sanitizeFcmError(response.status, errorBody)
      if (response.status === 404 || response.status === 400) return { sent: false, reason: lastReason, permanent: true }
      if (response.status !== 429 && response.status < 500) return { sent: false, reason: lastReason, permanent: true }
    } catch (error) {
      lastReason = error instanceof Error ? error.message.slice(0, 180) : 'FCM_NETWORK_ERROR'
    }
    await new Promise(resolve => setTimeout(resolve, 300 * (attempt + 1) ** 2))
  }
  return { sent: false, reason: lastReason, permanent: false }
}

async function sendNative(tokens:string[],title:string,body:string,data:Record<string,string>,type:string){
  if(!tokens.length) return {configured:Boolean(firebaseServiceAccount),attempted:false,sent:0,failed:0,reason:'NO_NATIVE_TOKENS'}
  if(!firebaseServiceAccount) return {configured:false,attempted:false,sent:0,failed:tokens.length,reason:'FIREBASE_SERVICE_ACCOUNT_JSON_MISSING'}
  try{
    const service=JSON.parse(firebaseServiceAccount) as {project_id?:string;client_email?:string;private_key?:string}
    if(!service.project_id||!service.client_email||!service.private_key) return {configured:false,attempted:false,sent:0,failed:tokens.length,reason:'FIREBASE_SERVICE_ACCOUNT_INVALID'}
    const accessToken=await googleAccessToken(service)
    let sent=0, failed=0, firstReason=''
    for(const token of tokens){
      const result=await sendTokenWithRetry(token,title,body,data,type,service.project_id,accessToken)
      if(result.sent){sent++; continue}
      failed++
      if(!firstReason) firstReason=result.reason
      if(result.permanent && (result.reason.includes('FCM_404') || result.reason.includes('UNREGISTERED') || result.reason.includes('registration-token-not-registered'))){
        await admin.from('native_push_tokens').delete().eq('token',token)
      }
    }
    return {configured:true,attempted:true,sent,failed,reason:firstReason||undefined}
  }catch(error){
    const reason=error instanceof Error?error.message.slice(0,220):'NATIVE_FCM_EXCEPTION'
    console.warn(`native push delivery failed: ${reason}`)
    return {configured:true,attempted:true,sent:0,failed:tokens.length,reason}
  }
}
