...
async function sendNative(tokens:string[],title:string,body:string,data:Record<string,string>,type:string){if(!tokens.length)return{configured:Boolean(firebaseServiceAccount),attempted:false,sent:0,failed:0,reason:'NO_NATIVE_TOKENS'};if(!firebaseServiceAccount)return{configured:false,attempted:false,sent:0,failed:tokens.length,reason:'FIREBASE_SERVICE_ACCOUNT_JSON_MISSING'};try{const service=JSON.parse(firebaseServiceAccount) as{project_id?:string;client_email?:string;private_key?:string};if(!service.project_id||!service.client_email||!service.private_key)return{configured:false,attempted:false,sent:0,failed:tokens.length,reason:'FIREBASE_SERVICE_ACCOUNT_INVALID'};const accessToken=await googleAccessToken(service);let sent=0;let failed=0;let firstReason='';for(const token of tokens){const message = type === 'call'
  ...
  : {
      ...
      android: {
        priority: 'HIGH',
        notification: { channel_id: 'yomy_default', sound: 'default' },
      },
    };
...
const {data:profiles, error:profilesError}=await admin.from('profiles').select('id,sleep_mode_enabled,sleep_start,sleep_end,timezone_name').in('id',targets);
if(profilesError)return json(500,{error:'TARGET_PROFILE_LOOKUP_FAILED',detail:profilesError.message.slice(0,180)});
const activeTargets=(profiles||[]).filter(profile => {
  if(!profile.sleep_mode_enabled) return true;
  try {
    const clock=new Intl.DateTimeFormat('en-GB',{hour:'2-digit',minute:'2-digit',hour12:false,timeZone:profile.timezone_name||'UTC'}).format(new Date());
    const minutes=Number(clock.slice(0,2))*60+Number(clock.slice(3,5));
    const start=Number(String(profile.sleep_start||'22:00').slice(0,2))*60+Number(String(profile.sleep_start||'22:00').slice(3,5));
    const end=Number(String(profile.sleep_end||'05:00').slice(0,2))*60+Number(String(profile.sleep_end||'05:00').slice(3,5));
...
if(type==='notification'){
  ...
  if(!message||message.sender_id!==user.id||message.receiver_id!==targetId||message.deleted_for_everyone)return json(403,{error:'MESSAGE_PUSH_NOT_AUTHORIZED',message:'The message is not owned by the sender or no longer deliverable'});
}else if(type==='call'){
  const{data:call,error:callError}=await admin.from('call_sessions').select('id,caller_id,callee_id,status').eq('id',String(data.call_id||'')).maybeSingle();
  if(callError)return json(500,{error:'CALL_LOOKUP_FAILED',detail:callError.message.slice(0,180)});
  if(!call||call.caller_id!==user.id||call.callee_id!==targetId||call.status!=='ringing')return json(403,{error:'CALL_PUSH_NOT_AUTHORIZED',message:'The call is no longer ringing or the caller/recipient is invalid'});
}else if(!['post','story'].includes(type))return json(400,{error:'UNSUPPORTED_PUSH_TYPE',message:'Unsupported push type'});
if(!activeTargets.length)return json(200,{webSent:0,nativeSent:0,nativeFailed:0,nativeConfigured:Boolean(firebaseServiceAccount),nativeAttempted:false,nativeReason:'SLEEP_MODE',recipients:0});const payloadData=toStringRecord(data);let webSent=0;if(vapidSubject&&vapidPublicKey&&vapidPrivateKey){webpush.setVapidDetails(vapidSubject,vapidPublicKey,vapidPrivateKey);const{data:subscriptions}=await admin.from('push_subscriptions').select('id,endpoint,p256dh,auth').in('user_id',activeTargets);const payload=JSON.stringify({type,title:String(body.title),body:String(body.body),tag:`${type}-${payloadData.call_id||payloadData.message_id||payloadData.post_id||payloadData.story_id||Date.now()}`,data:payloadData});for(const subscription of subscriptions||[]){try{await webpush.sendNotification({endpoint:subscription.endpoint,keys:{p256dh:subscription.p256dh,auth:subscription.auth}},payload,{TTL:type==='call'?45:300});webSent++}catch(error){const statusCode=error&&typeof error==='object'&&'statusCode'in error?Number((error as{statusCode:number}).statusCode):0;if(statusCode===404||statusCode===410)await admin.from('push_subscriptions').delete().eq('id',subscription.id)}}}const{data:nativeTokens}=await admin.from('native_push_tokens').select('token').in('user_id',targets);const native=await sendNative((nativeTokens||[]).map(row=>row.token),String(body.title),String(body.body),payloadData,type);await admin.from('push_delivery_diagnostics').insert({actor_id:user.id,push_type:type,target_count:activeTargets.length,token_count:nativeTokens?.length||0,firebase_configured:native.configured,fcm_attempted:native.attempted,fcm_sent:native.sent,fcm_failed:native.failed,fcm_reason:native.reason||null});if(native.failed>0&&native.sent===0&&nativeTokens?.length)return json(502,{error:'Native push delivery failed',nativeConfigured:native.configured,nativeAttempted:native.attempted,nativeSent:native.sent,nativeFailed:native.failed,nativeReason:native.reason||'FCM delivery failed',webSent,recipients:activeTargets.length});return json(200,{webSent,nativeSent:native.sent,nativeFailed:native.failed,nativeConfigured:native.configured,nativeAttempted:native.attempted,nativeReason:native.reason||null,recipients:targets.length})})
