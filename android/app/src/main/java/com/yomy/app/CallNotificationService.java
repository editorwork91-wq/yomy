...
public class CallNotificationService extends Service {
    ...
    private Notification buildNotification(String title, String body, String kind, String callId) {
        ...
        Notification.Builder builder = Build.VERSION.SDK_INT >= Build.VERSION_CODES.O ? new Notification.Builder(this, CHANNEL_ID) : new Notification.Builder(this).setPriority(Notification.PRIORITY_HIGH);
        builder.setSmallIcon(R.mipmap.ic_launcher).setContentTitle(title).setContentText(body).setCategory(Notification.CATEGORY_CALL).setVisibility(Notification.VISIBILITY_PRIVATE).setOngoing(true).setAutoCancel(false).setOnlyAlertOnce(true).setShowWhen(true).setContentIntent(incomingScreen);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) { try { builder.setFullScreenIntent(incomingScreen, true); } catch (Exception ignored) {} }
    ...
    private android.os.Bundle videoExtras() { android.os.Bundle extras = new android.os.Bundle(); extras.putBoolean(Notification.EXTRA_CALL_IS_VIDEO, true); return extras; }
    private Intent actionIntent(String action, String callId) { return new Intent(this, CallActionReceiver.class).setAction(action).putExtra(CallActionReceiver.EXTRA_CALL_ID, callId); }
    ...
    private void startPlaybackRespectingSystemMode() {
        try {
            AudioManager audio = (AudioManager) getSystemService(Context.AUDIO_SERVICE); if (audio == null) return;
            int mode = audio.getRingerMode(); if (mode == AudioManager.RINGER_MODE_SILENT) return;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) { NotificationManager nm = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE); if (nm != null && nm.getCurrentInterruptionFilter() != NotificationManager.INTERRUPTION_FILTER_ALL) return; }
    ...
    private void playDefaultRingtone() {
        try {
            Uri uri = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_RINGTONE); if (uri == null) uri = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION);
            ringtonePlayer = new MediaPlayer(); ringtonePlayer.setAudioAttributes(new AudioAttributes.Builder().setUsage(AudioAttributes.USAGE_NOTIFICATION_RINGTONE).setContentType(AudioAttributes.CONTENT_TYPE_SONIFICATION).build()); ringtonePlayer.setDataSource(this, uri); ringtonePlayer.setLooping(true); ringtonePlayer.prepare(); ringtonePlayer.start();
        } catch (Exception ignored) { stopPlaybackOnly(); }
    }
    private void startVibration() {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) { VibratorManager vm = (VibratorManager) getSystemService(Context.VIBRATOR_MANAGER_SERVICE); vibrator = vm == null ? null : vm.getDefaultVibrator(); }
            else vibrator = (Vibrator) getSystemService(Context.VIBRATOR_SERVICE);
            if (vibrator == null || !vibrator.hasVibrator()) return;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) vibrator.vibrate(VibrationEffect.createWaveform(VIBRATION_PATTERN, 0)); else vibrator.vibrate(VIBRATION_PATTERN, 0);
        } catch (Exception ignored) {}
    }
    private void stopPlaybackOnly() {
        handler.removeCallbacks(timeout);
        if (ringtonePlayer != null) { try { if (ringtonePlayer.isPlaying()) ringtonePlayer.stop(); } catch (Exception ignored) {} try { ringtonePlayer.release(); } catch (Exception ignored) {} ringtonePlayer = null; }
        if (vibrator != null) { try { vibrator.cancel(); } catch (Exception ignored) {} vibrator = null; }
    }
    private void stopRinging() {
        stopPlaybackOnly();
        try { NotificationManager manager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE); if (manager != null) manager.cancel(NOTIFICATION_ID); } catch (Exception ignored) {}
    ...
    @Override public void onDestroy() { stopPlaybackOnly(); super.onDestroy(); }
