...
public class YomyFirebaseMessagingService extends FirebaseMessagingService {
    @Override public void onMessageReceived(RemoteMessage message) {
        ...
        try { CallNotificationService.start(this, callId, title, body, kind); } catch (Exception ignored) {}
    }
    @Override public void onNewToken(String token) { super.onNewToken(token); }
    private static String value(Map<String, String> data, String key) { String value = data.get(key); return value == null ? "" : value.trim(); }
    private static String first(Map<String, String> data, String a, String b) { String value = value(data, a); return value.isEmpty() ? value(data, b) : value; }
}
