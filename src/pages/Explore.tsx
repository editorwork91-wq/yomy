...
export default function Explore() {
  ...
  const fetchExplore = useCallback(async () => {
    setLoading(true)
    const { data, error } = await supabase.from('posts').select('*, profiles!user_id(id, username, full_name, avatar_url, is_verified, is_private), likes(user_id), post_tags(tag)').eq('status', 'published').order('published_at', { ascending: false }).limit(60)
    if (error) console.error('Explore posts failed:', error.message)
    setPosts((data || []).map(p => ({ ...p, _likes_count: p.likes?.length || 0, _liked_by_me: p.likes?.some((l: { user_id: string }) => l.user_id === user?.id) || false, _tags: p.post_tags?.map((t: { tag: string }) => t.tag) || [] })))
    setLoading(false)
  }, [user])
  ...
  const fetchSuggested = useCallback(async () => {
    ...
    const { data: suggestions, error } = await supabase.from('profiles').select('*').not('id', 'in', `(${excluded.join(',')})`).limit(10)
  ...
  const handleSearch = async () => {
    ...
    if (!term) { setUsers([]); setSearchPosts([]); return }
    setSearching(true)
    ...
    setUsers(userData || [])
    const merged: Post[] = []; const seen = new Set<string>()
    for (const row of tagPosts || []) { const p = row.posts as unknown as Post; if (p?.id && !seen.has(p.id)) { seen.add(p.id); merged.push(p) } }
    for (const p of textPosts || []) { if (p?.id && !seen.has(p.id)) { seen.add(p.id); merged.push(p as unknown as Post) } }
    setSearchPosts(merged); setSearching(false)
  }

  const handleFollow = async (targetId: string) => {
    if (!user) return
    setActionError(null)
    const previous = followStates[targetId] || 'none'
    setFollowStates(s => ({ ...s, [targetId]: 'pending' }))
    const { data, error } = await supabase.rpc('follow_user', { p_target_id: targetId })
    if (error || !data) {
      setFollowStates(s => ({ ...s, [targetId]: previous }))
      setActionError(error?.message || 'Could not update follow status')
      return
    }
  ...
  return (
    <div className="pb-20">
      ...
      <div className="max-w-lg mx-auto">
        ...
        {actionError && <p className="px-4 pb-2 text-center text-xs text-destructive" role="status">{actionError}</p>}
