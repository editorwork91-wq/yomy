...
export default function Feed() {
  ...
  const fetchPosts = useCallback(async (pageNum: number) => {
    ...
    if (error) {
      console.error('Feed posts failed:', error.message)
    ...
    const enriched = (data || []).map(p => ({
      ...
      _saved_by_me: p.saved_posts?.some((s: { post_id: string }) => s.post_id === p.id) || false,
        ...
        {loading && posts.length === 0 ? (
          ...
          <Empty className="mt-12">
            <EmptyHeader>
              ...
              <EmptyDescription>
                Follow people to see their posts here. <Link to="/explore" className="text-primary">Explore</Link> to find accounts.
              </EmptyDescription>
            </EmptyHeader>
          </Empty>
        ) : (
          <>
            {posts.map(post => (
              <PostCard key={post.id} post={post} onDeleted={id => setPosts(ps => ps.filter(p => p.id !== id))} />
            ))}
            {loading && <div className="flex items-center justify-center h-16"><Spinner className="size-5" /></div>}
            {!hasMore && posts.length > 0 && <p className="text-center text-sm text-muted-foreground py-8">You're all caught up!</p>}
          </>
        )}
      </div>
    </div>
