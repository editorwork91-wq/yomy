...
export default function Fedo() {
  ...
  useEffect(() => {
    ...
    if (!item || !user) return
    void supabase.rpc('record_fedo_event', { p_fedo_id: item.id, p_event_type: 'view', p_watch_ms: 0, p_meta: { source: 'fedo_feed' } })
  }, [index, items, user])
  ...
  const toggleLike = async () => {
    ...
    const item = items[index]
    const { data: existing } = await supabase.from('fedo_likes').select('fedo_id').eq('fedo_id', item.id).eq('user_id', user.id).maybeSingle()
    if (existing) {
      await supabase.from('fedo_likes').delete().eq('fedo_id', item.id).eq('user_id', user.id)
      await supabase.rpc('record_fedo_event', { p_fedo_id: item.id, p_event_type: 'like', p_meta: { action: 'remove' } })
    } else {
      await supabase.from('fedo_likes').insert({ fedo_id: item.id, user_id: user.id })
      await supabase.rpc('record_fedo_event', { p_fedo_id: item.id, p_event_type: 'like', p_meta: { action: 'add' } })
    }
  }
  ...
  const toggleSave = async () => {
    if (!user || !items[index]) return
    const item = items[index]
    const { data: existing } = await supabase.from('fedo_saves').select('fedo_id').eq('fedo_id', item.id).eq('user_id', user.id).maybeSingle()
    if (existing) await supabase.from('fedo_saves').delete().eq('fedo_id', item.id).eq('user_id', user.id)
    else await supabase.from('fedo_saves').insert({ fedo_id: item.id, user_id: user.id })
    await supabase.rpc('record_fedo_event', { p_fedo_id: item.id, p_event_type: 'save', p_meta: { action: existing ? 'remove' : 'add' } })
  }
  ...
  const share = async () => {
    ...
    try { if (navigator.share) await navigator.share({ title: 'Yomy Fedo', text: item.caption, url }); else { await navigator.clipboard.writeText(url); toast.success('Link copied') } } catch {}
  ...
  const uploadFedo = async (file: File) => {
    ...
    try {
      ...
      if (ticketError || !ticket?.storage_url || !ticket?.path || !ticket?.token || !ticket?.anon_key) throw ticketError || new Error('Upload ticket unavailable')
      const shardClient = createClient(String(ticket.storage_url), String(ticket.anon_key))
      const direct = `${String(ticket.storage_url).replace(/\/$/, '')}.storage.supabase.co`
      const endpoint = direct.includes('.storage.supabase.co') ? `${direct}/storage/v1/upload/resumable` : `${String(ticket.storage_url).replace(/\/$/, '')}/storage/v1/upload/resumable`
    ...
    <div className="h-[calc(100dvh-3.5rem)] relative touch-none" onPointerDown={e => { startRef.current = { x: e.clientX, y: e.clientY }; e.currentTarget.setPointerCapture(e.pointerId) }} onPointerUp={e => { const dx = e.clientX - startRef.current.x; const dy = e.clientY - startRef.current.y; if (Math.abs(dx) > 80 && Math.abs(dx) > Math.abs(dy)) move(dx < 0 ? 1 : -1); else if (dy < -80) openCreator() }}>
      {loading ? <div className="h-full flex items-center justify-center"><Spinner className="size-8"/></div> : !current ? <div className="h-full flex flex-col items-center justify-center gap-4 text-white/70"><Play className="size-10"/><p>No public Fedos yet.</p><Button onClick={() => fileRef.current?.click()}>Publish the first Fedo</Button></div> : <div className="h-full relative bg-black">
        ...
        <div className="absolute right-3 bottom-8 flex flex-col items-center gap-5">
          <Button variant="ghost" size="icon" className="rounded-full size-12 bg-black/30 text-white" onClick={() => void toggleLike()}><Heart className="size-6"/></Button>
          <Button variant="ghost" size="icon" className="rounded-full size-12 bg-black/30 text-white"><MessageCircle className="size-6"/></Button>
          <Button variant="ghost" size="icon" className="rounded-full size-12 bg-black/30 text-white" onClick={() => void share()}><Share2 className="size-6"/></Button>
          <Button variant="ghost" size="icon" className="rounded-full size-12 bg-black/30 text-white" onClick={() => void toggleSave()}><Bookmark className="size-6"/></Button>
          <Button variant="ghost" size="icon" className="rounded-full size-12 bg-black/30 text-white" onClick={() => toast.info('Report submitted')}><Flag className="size-5"/></Button>
        </div>
