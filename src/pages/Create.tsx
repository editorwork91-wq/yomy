...
import { ImagePlus, X, ChevronDown, Globe, Lock, Users, Clapperboard } from 'lucide-react'
...
export default function Create() {
  ...
  const handlePost = async () => {
    ...
    addTagsFromInput()
    const finalTags = Array.from(new Set([...tags, ...tagInput.split(',').map(normalizeTag).filter(Boolean)])).map(t => t.slice(0, MAX_TAG_LENGTH)).slice(0, MAX_TAGS)
    postingRef.current = true
    setPosting(true)
    try {
      const { data: existing, error: existingError } = await supabase.from('posts').select('id').eq('idempotency_key', idempotencyRef.current).maybeSingle()
      ...
      if (finalTags.length) {
        const { error: tagError } = await supabase.from('post_tags').upsert(finalTags.map(tag => ({ post_id: postId, tag })), { onConflict: 'post_id,tag', ignoreDuplicates: true })
