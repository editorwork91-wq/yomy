...
import { Send, ImagePlus, Eye, EyeOff, Lock, Mic, Phone, Video, MoreVertical, Trash2, Volume2, Ban, Reply, Copy, Pencil, X, MicOff, Play, Square, VolumeX } from 'lucide-react'
...
export default function Chat() {
  ...
  const sendTextMessage = async () => {
    ...
    try {
      ...
      if (insertedMessage) {
        setMessages(prev => prev.some(m => m.id === insertedMessage.id) ? prev : [...prev, {
          id: insertedMessage.id, sender_id: user.id, receiver_id: otherUser.id, content: newMessage.trim(), media_url: '', media_type: '',
          is_seen: false, is_encrypted: true, view_once: false, view_once_opened: false, deleted_at: null, deleted_for_everyone: false,
  ...
  return (
    <div className="flex flex-col h-screen">
      <TopBar title="" showBack right={<div className="flex items-center gap-1">
        <Button variant="ghost" size="icon" className="size-9" onClick={() => void startCall(otherUser, 'voice')} aria-label="Voice call"><Phone className="size-5" /></Button>
        <Button variant="ghost" size="icon" className="size-9" onClick={() => void startCall(otherUser, 'video')} aria-label="Video call"><Video className="size-5" /></Button>
        <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="size-9"><MoreVertical className="size-5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => void toggleMute}><Volume2 className="size-4 mr-2" />{isMuted ? 'Unmute' : 'Mute'} notifications</DropdownMenuItem><DropdownMenuItem onClick={() => void clearChat()}><Trash2 className="size-4 mr-2" />Clear chat</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={() => void blockUser()} className="text-destructive focus:text-destructive"><Ban className="size-4 mr-2" />Block user</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
      </div>} />
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-2">
        {messages.length === 0 ? <div className="flex flex-col items-center justify-center h-full gap-3 text-center"><Avatar className="size-20"><AvatarImage src={otherUser.avatar_url} /><AvatarFallback className="text-2xl">{otherUser.username[0]?.toUpperCase()}</AvatarFallback></Avatar><div><p className="font-semibold">{otherUser.username}</p><p className="text-sm text-muted-foreground">{otherUser.full_name || ''}</p></div><Button size="sm" onClick={() => setNewMessage('Hi! 👋')}>Say hello</Button></div> : messages.map((msg, idx) => {
          ...
          return <div key={msg.id}>{showDate && <div className="flex justify-center my-3"><span className="text-xs text-muted-foreground bg-muted px-3 py-1 rounded-full">{format(new Date(msg.created_at), 'MMM d, yyyy')}</span></div>}
            <div className={cn('flex', isMe ? 'justify-end' : 'justify-start')}><div className={cn('max-w-[78%] rounded-2xl px-3 py-2 relative', isMe ? 'bg-primary text-primary-foreground' : 'bg-muted', msg.deleted_for_everyone && 'opacity-60')} onClick={() => handleMessageTap(msg)} onContextMenu={e => { e.preventDefault(); setShowLongPressMenu(msg.id) }}>
              {msg.deleted_for_everyone ? <p className="text-sm italic opacity-60">Message deleted</p> : msg.view_once && msg.media_url && !msg.view_once_opened ? <button onClick={() => void openViewOnce(msg)} className="flex items-center gap-2"><Eye className="size-4" /><span className="text-sm">View-once media</span></button> : msg.view_once && msg.view_once_opened ? <div className="flex items-center gap-2 text-muted-foreground"><EyeOff className="size-4" /><span className="text-sm italic">Media expired</span></div> : <>
                {msg.reply_to ? <div className={cn('mb-1 px-2 py-1 rounded text-xs opacity-70', isMe ? 'bg-primary-foreground/10' : 'bg-black/10')}><Reply className="size-3 inline mr-1" />{msg.reply_to.content || (msg.reply_to.media_type === 'image' ? '📷 Photo' : msg.reply_to.media_type === 'video' ? '🎬 Video' : msg.reply_to.media_type === 'audio' ? '🎙️ Voice' : '📎 Media')}</div> : null}
                {msg.media_url && msg.media_type === 'image' && <button onClick={() => msg.view_once ? void openViewOnce(msg) : setShowViewOnce(msg.media_url)} className="block"><img src={msg.media_url} alt="" className="rounded-xl max-w-64 max-h-80 object-cover mb-1" loading="lazy" /></button>}
                {msg.media_url && msg.media_type === 'video' && <video src={msg.media_url} controls playsInline className="rounded-xl max-w-64 max-h-80 mb-1" />}
                {msg.media_url && msg.media_type === 'audio' && <div className="flex items-center gap-2 py-1 min-w-[190px]"><Mic className="size-4 shrink-0" /><audio src={msg.media_url} controls className="h-9 w-full" /></div>}
      ...
      <div className="border-t p-3 pb-safe flex items-center gap-2">
        <input ref={fileRef} type="file" accept="image/*,video/*,audio/*" className="hidden" onChange={e => { const file = e.target.files?.[0]; if (file) selectMedia(file); e.currentTarget.value = '' }} />
        <Button variant="ghost" size="icon" className="size-9 shrink-0" onClick={() => fileRef.current?.click()} disabled={uploadingMedia || recording || !!pendingMedia}><ImagePlus className="size-5" /></Button>
        <Button variant="ghost" size="icon" className={cn('size-9 shrink-0', pendingMedia?.kind === 'audio' && 'text-primary')} onClick={() => void startRecording()} disabled={uploadingMedia || recording || !!pendingMedia} aria-label="Record voice"><Mic className="size-5" /></Button>
        <Button variant="ghost" size="icon" className={cn('size-9 shrink-0', viewOnceMode && 'text-primary')} onClick={() => setViewOnceMode(value => !value)} disabled={recording || !!pendingMedia}>{viewOnceMode ? <Eye className="size-5" /> : <EyeOff className="size-5" />}</Button>
        <Input placeholder="Message..." value={newMessage} onChange={e => setNewMessage(e.target.value)} disabled={recording || !!pendingMedia} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void sendTextMessage() } }} className="flex-1" />
        <Button size="icon" className="size-9 shrink-0" disabled={!newMessage.trim() || sending || recording || !!pendingMedia} onClick={() => void sendTextMessage()}>{sending ? <Spinner className="size-4" /> : <Send className="size-4" />}</Button>
      </div>
