...
type YomyEvent = {
  ...
  event_type: string
  priority: 0 | 1 | 2 | 3
  source_table: 'messages' | 'notifications' | 'call_sessions' | 'reminders'
  source_id: string
...
function readCursor(): string | null { try { return localStorage.getItem(CURSOR_KEY) } catch { return null } }
...
function persistRecent(ids: Set<string>) { try { localStorage.setItem(RECENT_KEY, JSON.stringify(Array.from(ids).slice(-MAX_RECENT))) } catch {} }
...
export default function YomyEventEngine() {
  ...
  const remember = useCallback((event: YomyEvent) => {
    ...
    if (!cursorRef.current || event.created_at > cursorRef.current) { cursorRef.current = event.created_at; writeCursor(event.created_at) }
  }, [])
  ...
  const flushBucket = useCallback(async (key: string) => {
    ...
    showYomyLocalNotification(bucket.actorNames[0] || 'Yomy', activitySummary(eventType, bucket.count, bucket.actorNames), 'message', '/notifications')
  }, [])
  ...
  const queueActivity = useCallback(async (event: YomyEvent, actorName: string) => {
    ...
    const timer = window.setTimeout(() => { timersRef.current.delete(timer); void flushBucket(key) }, AGGREGATE_WINDOW_MS)
  ...
  const reconcile = useCallback(async (presentNew: boolean) => {
    ...
    try {
      ...
      let query = supabase.from('notification_events').select('id,recipient_id,actor_id,event_type,priority,source_table,source_id,entity_id,deep_link,payload,created_at').eq('recipient_id', user.id).order('created_at', { ascending: true }).limit(200)
      ...
      for (const event of events) { if (presentNew) await present(event); else remember(event) }
    } finally { processingRef.current = false }
  }, [present, remember, user])

  useEffect(() => {
    if (!user) return
    recentIdsRef.current = readRecent(); cursorRef.current = readCursor(); void reconcile(Boolean(cursorRef.current))
    ...
    const channel = supabase.channel(`yomy-event-engine-${user.id}`).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notification_events', filter: `recipient_id=eq.${user.id}` }, payload => { void present(payload.new as YomyEvent) }).subscribe(status => { if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') void reconcile(true) })
    return () => {
      window.removeEventListener('online', onOnline); document.removeEventListener('visibilitychange', onVisibility)
      timersRef.current.forEach(timer => window.clearTimeout(timer)); timersRef.current.clear(); bucketsRef.current.clear()
