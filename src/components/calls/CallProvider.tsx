...
export default function CallProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`calls-${user.id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'call_sessions', filter: `callee_id=eq.${user.id}` }, async payload => {
        ...
        const loaded = await loadIncomingById(call.id)
        if (loaded && !activeRef.current) openCallRoute(loaded.callerProfile, loaded.call.id)
      })
  ...
  useEffect(() => {
    ...
    const onNativeAction = (event: Event) => {
      ...
      if (detail?.action && detail.callId) void handleNativeAction(detail.action, detail.callId)
    }
  ...
  }, [location.search, loadIncomingById, user])
